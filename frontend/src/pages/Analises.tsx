  ...
  useEffect(() => {
    ...
    const fetchStats = async () => {
      ...
      try {
        ...
        const exerciciosComMedia: ExercicioStats[] = Array.from(
          statsMap.values(),
        ).map((stat) => ({
          nome: stat.nome,
          total: stat.total,
        ...
        setExercicioStats(exerciciosComMedia.sort((a, b) => b.total - a.total));
      } catch (err) {
        const error = err as {
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
                <div className="bg-white p-6 rounded-lg shadow">
                  ...
                  <p className="text-3xl font-bold mt-2">
                    {formatarCarga(stats.cargaMedia)}kg
                  </p>
                </div>
                <div className="bg-white p-6 rounded-lg shadow">
                  <p className="text-gray-600 text-sm">Carga Maxima</p>
                  <p className="text-3xl font-bold mt-2">
                    {formatarCarga(stats.cargaMaxima || 0)}kg
                  </p>
                </div>
              </div>
            )}
            ...
            {exercicioStats.length > 0 && (
              <div className="bg-white p-6 rounded-lg shadow">
                <h2 className="text-xl font-bold mb-4">
                  Analise por Exercicio
                </h2>
                <div className="space-y-4">
                  {exercicioStats.map((ex) => (
                    <div
                      key={ex.nome}
                      className="p-4 bg-gray-50 rounded border-l-4 border-purple-500"
                    >
                      <div className="flex justify-between items-start mb-2">
                        <div>
                          <p className="font-semibold text-lg">{ex.nome}</p>
                          <p className="text-sm text-gray-500">
                            Ultimo: {formatarData(ex.dataUltimo)}
                          </p>
                        </div>
                      ...
                      <div className="grid grid-cols-2 gap-4">
                        <div>
                          ...
                          <p className="text-2xl font-bold text-purple-600">
                            {formatarCarga(ex.maiorCarga)}kg
                          </p>
                        </div>
                        <div>
                          <p className="text-xs text-gray-500">Media</p>
                          <p className="text-2xl font-bold text-blue-600">
                            {formatarCarga(ex.mediaCarga)}kg
                          </p>
                        </div>
                      </div>
                    </div>
